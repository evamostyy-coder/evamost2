...
export default function DigitsPage() {
  ...
  // Bot — passive consumer of the shared trading environment.
  // Reuses: currentTick, prices (tick stream), proposal (proposal stream),
  //         setContractMode + setSelectedDigit (UI setters), tradingWs (same connection).
  // Creates: zero new WebSocket subscriptions or proposal requests.
  const bot = useBot({
    ws: trading.tradingWs,
    isConnected,
    isAuthenticated: !!auth.wsUrl,
  ...
  return (
    <DigitsView
      ...
      bot={bot}
      proposalReady={!!trading.proposal}
    />
  );
}
