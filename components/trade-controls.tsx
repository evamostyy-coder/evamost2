...
interface TradeControlsProps {
  ...
  /** When true (bot running), contract type selector is locked. */
  isLocked?: boolean;
}
  ...
  return (
    <div className="space-y-2 sm:space-y-4">
      <ToggleGroup
        ...
        value={contractMode}
        onValueChange={value => {
          if (value && !isLocked) onContractModeChange(value as ContractMode);
        }}
        className={`w-full gap-0 rounded-full bg-muted p-1${isLocked ? ' opacity-50 pointer-events-none' : ''}`}
      >
        {modeOptions.map(opt => (
          <ToggleGroupItem
            key={opt.value}
